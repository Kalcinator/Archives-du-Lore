...
#region Logic
try {
    Write-Host "Initialisation des Archives du Lore..." -ForegroundColor Cyan
    ...
    Write-Warning "⚠️  Interruption détectée ou erreur système : $($_.Exception.Message) ⚠️"
}
finally {
    # Nettoyage natif PowerShell (Robuste sur Windows)
    try {
        Set-Location -Path $ProjectRoot
        Write-Host "`n Nettoyage des archives..." -ForegroundColor Yellow
        if (Test-Path -Path "dist") {
            Remove-Item -Path "dist" -Recurse -Force
        }
        Write-Host "Fin de mission." -ForegroundColor Cyan
    }
    catch {
        Write-Warning "⚠️ Le nettoyage a rencontré un obstacle ⚠️ "
    }
}
#endregion
